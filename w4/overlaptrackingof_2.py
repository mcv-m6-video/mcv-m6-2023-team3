...
def task2_1(path="./w4/boxesScores1.pkl", video_path= './AICity_data/train/S03/c010/vdo.avi'):
    start_frame = 218
    final_frame = 230
    pkl_path = path
    ...
    frame_boundingboxes = aux_frame_boxes[start_frame:final_frame]

    # Once we have done the detection we can start with the tracking
    cap = cv2.VideoCapture(video_path)
    i = 0
    while i!= start_frame-1:
    ...
    for i in trange(len(frame_boundingboxes), desc="Video"):
        vidCapture.set(cv2.CAP_PROP_POS_FRAMES, start_frame+i)
    ...
    # Load gt for plot
    gt = load_bb(path=r"D:\\Documentos\\Puthon\\MCV\\M6\\mcv-m6-2023-team3\\AICity_data\\train\\S03\\c010\\gt\\gt.txt")
    ...
    for Nframe in trange(len(frame_boundingboxes), desc="Evaluation"):
        Nframe_new = Nframe + start_frame
        # get the ids of the tracks from the ground truth at this frame
        gt_list = [item[1] for item in gt if item[0] == Nframe_new]
        ...
        for i in range(len(gt_list)):
            ...
            bboxGT = gt_list[i]
            bboxGT = [item[3:7] for item in gt if (item[0] == Nframe_new and item[1] == bboxGT)]
